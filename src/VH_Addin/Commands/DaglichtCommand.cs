...
namespace VH_Tools.Commands
{
    ...
    public class DaglichtCommand : IExternalCommand
    {
        public Result Execute(ExternalCommandData commandData, ref string message, ElementSet elements)
        {
            ...
            try
            {
                ...
                using (Transaction trans = new Transaction(doc, "VH Daglichttool"))
                {
                    ...
                    // Note: WarningSwallower might need migration if not present
                    ...
                    try
                    {
                        ...
                        if (options.DoGlass)
                        {
                            ...
                            RunNENCalculation(doc, windows, results, options.DoExport);
                        }
        ...
        private void RunNENCalculation(Document doc, List<FamilyInstance> windows, List<WindowResult> results, bool doExport)
        {
            var nenCalc = new NENCalculator(doc);
            nenCalc.Calculate(windows, results, doExport);
        }
    }
}
