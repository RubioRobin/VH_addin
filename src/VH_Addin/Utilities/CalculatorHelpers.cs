...
namespace VH_Tools.Utilities
{
    public static class CalculatorHelpers
    {
        ...
        public static (double zRefFt, double zRefMm) GetGlazingRefHeight(
            ...
            BoundingBoxXYZ bbox,
            Level level,
            double sashOffsetFt = 0,
            double defaultFrameHeightFt = 0,
            double defaultGlassOffsetFt = 0)
        {
            double zLevelFt = level?.Elevation ?? (bbox?.Min.Z ?? 0);

            // Helper to check if parameter exists (Instance or Type)
            bool HasParameter(string paramName)
            {
                if (window.LookupParameter(paramName) != null)
                    return true;
                ElementType type = window.Document.GetElement(window.GetTypeId()) as ElementType;
                return type?.LookupParameter(paramName) != null;
            }

            bool hasAlu = HasParameter(AL_SIDE) ||
                         HasParameter(AL_TOP_BOT) ||
                         HasParameter(AL_OFF_SIDE) ||
                         HasParameter(AL_VIEW_SILL) ||
                         HasParameter(AL_EXTRA_UNDER);
            ...
            {
                // If P_BOT is missing (generic window), assume standard frame height from UI
                double tBotFt = GetDoubleParam(window, P_BOT) ?? defaultFrameHeightFt;
                glassBottomRelFt = sillFt + tBotFt + defaultGlassOffsetFt;
            }

            // Apply sash offset (for operable windows) to the physical glass bottom
            double glassBottomAbsFt = zLevelFt + glassBottomRelFt + sashOffsetFt;
            double minRefAbsFt = zLevelFt + FT_600;

            // NEN 2057: Start at Max(600mm, GlassBottom)
            ...
            if (p != null)
            {
                try
                {
                    if (p.StorageType == StorageType.Double)
                    {
                        ...
                        // CRITICAL FIX: In Revit MM-version, custom Length parameters might return values in millimeters via AsDouble()
                        // Heuristic: If value > 10.0, it's interpreted as millimeters and converted to feet.
                        // This handles ALU parameters like offset_stijl (17mm), aanzicht_raamprofiel (78mm), etc.
                        if (val > 10.0)
                        {
                            return val * MM_TO_FT;
                        }
                        
                        // Value is already in feet (BuiltInParameters or feet-based projects)
                        return val;
                    }
        ...
        public static string GetStringParam(Element elem, string paramName)
        {
            if (string.IsNullOrEmpty(paramName) || elem == null)
                return null;
            ...
            if (p != null)
            {
                try
                {
                    ...
                    if (p.StorageType == StorageType.ElementId)
                    {
                        ...
                        {
                            Element nested = elem.Document.GetElement(id);
                            return nested?.Name;
                        }
                    }
                }
