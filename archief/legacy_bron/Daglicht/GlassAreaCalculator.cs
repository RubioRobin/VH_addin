...
namespace VH_DaglichtPlugin
{
    public class GlassAreaCalculator
    {
        ...
        public double Calculate(FamilyInstance window, BoundingBoxXYZ bbox, XYZ facing, double mmCut = 600.0)
        {
            ...
            // We use GetDoubleParam to check existence (returns null if not found)
            ...
            if (hasAlu)
            {
                ...
                double BEff = wParam.Value + 2.0 * offSide;
                double HEff = hParam.Value + 2.0 * offSide;
            ...
            try 
            {
                ...
                double glassTopZ = bbox.Max.Z - tTop; // Goal: cut from the top of the actual glass
                
                // Find a valid 3D view - prefer the default {3D} view or any non-template 3D view
                ...
                // Preparation for FilteredElementCollector fallback
                ...
                if (view3d != null) 
                {
                    ...
                    intersector.FindReferencesInRevitLinks = true; // Crucial for linked models

                    // 25-ray Grid Strategy (5x5 grid covering width and wall depth)
                    XYZ sideDir = facing.CrossProduct(XYZ.BasisZ).Normalize();
                    double winWidth = bbox.Max.DistanceTo(new XYZ(bbox.Min.X, bbox.Min.Y, bbox.Max.Z)) * 0.8; // Use 80% of width to avoid edge cases
                    
                    double[] widthSteps = { -0.5, -0.25, 0.0, 0.25, 0.5 }; 
                    double[] thickOffsets = { -1.0, -0.2, 0.2, 1.0, 2.5 }; // Range from way inside to far outside (ft)
                    ...
                    foreach (double wStep in widthSteps)
                    {
                        ...
                        foreach (double tOffset in thickOffsets)
                        {
                            ...
                            {
                                // If the reference is one of our excluded elements, skip it
                ...
                // FALLBACK: If raycasting hit nothing (maybe disabled categories in view),
                // use BoundingBox overlap as a safety net.
                if (maxCutVal <= 0)
                {
                    ...
                    foreach (var opt in potentials)
                    {
                        ...
                        // Check for axis-aligned planar overlap
                        if (obbox.Min.X < bbox.Max.X && obbox.Max.X > bbox.Min.X &&
                            obbox.Min.Y < bbox.Max.Y && obbox.Max.Y > bbox.Min.Y)
                        {
                            // If the bottom of this element is within the window's height
            ...
            // 8) Sash width (operable deduction)
            // If explicit sash width is missing, fallback to side profile width (common for wood)
        ...
        private double? GetSashWidthFromNested(FamilyInstance window)
        {
            ...
            // Strategy A: Check SubComponents (Shared Families)
            ...
            // Strategy B: Check Parameters (Linked Types)
            void CheckParameters(Element elem)
            {
                if (elem == null) return;
                
                // Use GetOrderedParameters() as it includes parameters visible in UI but sometimes hidden in .Parameters
                IList<Parameter> paramsToScan;
                try { paramsToScan = elem.GetOrderedParameters(); }
                catch { paramsToScan = elem.Parameters.Cast<Parameter>().ToList(); }
                ...
                foreach (Parameter p in paramsToScan)
                {
                    ...
                    // Optimization: check naming to avoid scanning every material/profile
                    bool isLikelyFilling = p.Definition.Name.ToLower().Contains("vlakvulling") || 
                                           sym.Name.ToLower().Contains("vlakvulling");

                    if (isLikelyFilling)
                    {
                        // Scan all parameters on the symbol for variations of glas_offset
                        IList<Parameter> symParams;
                        try { symParams = sym.GetOrderedParameters(); }
                        catch { symParams = sym.Parameters.Cast<Parameter>().ToList(); }
                        ...
                        // Fallback: explicit lookup (sometimes faster/more reliable)
            ...
            // --- STRATEGY C: Heuristic Fallback (Name based) ---
            // User request: If we can't read the parameter (because it's not shared),
            // but we recognize the name as an operable window, assume 69mm.
            if (maxSashFt <= 0.001)
            {
                // Re-scan symbols for names to apply default
                void CheckNames(Element elem)
                {
                    ...
                    foreach (Parameter p in paramsToScan)
                    {
                        ...
                        // Triggers for 69mm sash
                        bool isOperable = sName.Contains("draaikiep") || 
                                          sName.Contains("valraam") || 
                                          sName.Contains("uitzet") ||
                                          sName.Contains("stolp") ||
                                          (sName.Contains("draai") && !sName.Contains("draaiend_deel")); // Avoid duplicates if naming varies

                        if (isOperable)
                        {
                            // Use configured default (e.g. 69mm)
