    ...
    public partial class DaglichtCalculator
    {
            ...
            HashSet<long> assemblyWallIds)
        {
            ...
            var candidatesByDist = _allObstructions
                .Where(o => o.element.Id.Value != window.Id.Value && !assemblyWallIds.Contains(o.element.Id.Value))
                .Select(o => {
        ...
        private List<Solid> CollectSolids(GeometryElement geomElem)
        {
            ...
            foreach (GeometryObject gObj in geomElem)
            {
                ...
                {
                    // Create a pseudo-solid or just extract vertices later? 
                    // Better to just add mesh vertices to the point cloud in GetEdgePoints.
                }
                else if (gObj is GeometryInstance gi)
                {
                    GeometryElement instGeom = gi.GetInstanceGeometry();
                    foreach (GeometryObject gObj2 in instGeom)
                    {
                        if (gObj2 is Solid solid2 && solid2.Volume > 1e-6)
                        {
                            solids.Add(solid2);
                        }
                    }
                }
                // Handle direct Mesh objects (often found in Generic Models/Imports)
                else if (gObj is Mesh m)
                {
                    // We can't easily turn a mesh into a solid here, 
                    // but we can ensure its vertices are captured.
                }
            }
        ...
        public static HashSet<int> GetHostLikeWallIds(BoundingBoxXYZ bbox, List<Element> allObstructions)
        {
            var walls = allObstructions.OfType<Wall>().ToList();
            return CalculatorHelpers.GetHostLikeWallIds(bbox, walls);
        }

        // Glass area calculation will be in GlassAreaCalculator.cs
        private double? CalculateGlassArea(FamilyInstance window, BoundingBoxXYZ bbox)
        {
            var glassCalc = new GlassAreaCalculator(_doc, 69.0);
            double areaSqFt = glassCalc.Calculate(window, bbox, window.FacingOrientation);
            return areaSqFt; 
        }

        private void SetResultOnInstance(FamilyInstance window, double valueM2)
        {
            foreach (string paramName in TARGET_PARAMS)
            {
                Parameter p = window.LookupParameter(paramName);
                if (p != null && !p.IsReadOnly)
                {
                    try
                    {
                        ...
                        {
                            double valFt2 = valueM2 / SQFT_TO_SQM;
                            p.Set(valFt2);
                            return;
                        }
                        else if (p.StorageType == StorageType.String)
                        {
                            p.Set(valueM2.ToString("F2"));
                            return;
                        }
                    }
                    catch { }
