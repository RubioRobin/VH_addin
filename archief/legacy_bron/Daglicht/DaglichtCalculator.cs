    ...
    public partial class DaglichtCalculator
    {
            ...
            GraphicsStyle alpha3DStyle)
        {
            ...
            try
            {
                ...
                var (zRef, zRefMm) = GetGlazingRefHeight(window, bbox, level);
                ...
                // Find best host wall (interior-most among all candidates at this location)
                // We use a tolerance to find all leaves of a multi-layer wall assembly
                double wallSearchTolerance = 500.0 * MM_TO_FT; // 500mm
                var candidatesAtLoc = _allObstructions
                    .Where(o => o.element is Wall)
                    .Select(o => (Wall)o.element)
                    .Where(w => BBoxesIntersectWithTolerance(bbox, w.get_BoundingBox(null), wallSearchTolerance))
                    .ToList();

                XYZ start = centerXY;
                if (candidatesAtLoc.Any())
                {
                    start = GetStartOnInsideFace(candidatesAtLoc, centerXY, zRef, nDir);
                }

                // Collect IDs of walls in the assembly to exclude them from obstructions (self-shadowing)
                // We use long for ElementId compatibility across Revit versions (2024+)
                var assemblyWallIds = new HashSet<long>(candidatesAtLoc.Select(w => w.Id.Value));
                ...
                result.Message = $"α {(_options.DoAlpha ? "aan" : "uit")} / β {(_options.DoBeta ? "aan" : "uit")} / Glas {(_options.DoGlass ? "aan" : "uit")}";
